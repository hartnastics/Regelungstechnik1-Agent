...
Wie verändert sich F_W(s) wenn ein Messglied G_M(s) \neq 1 im Rückführzweig liegt?
...
Warum ist „alle Koeffizienten positiv" für n \geq 3 allein nicht hinreichend?
...
Was ist die Durchtrittsfrequenz \omega_D und wie findet man sie?
Was ist die Phasenschnittfrequenz \omega_k und wie findet man sie?
Wie hängt das Bode-Diagramm mit der Ortskurve zusammen?
Wie kann man aus dem Bode-Diagramm Stabilität beurteilen?
Was bedeutet es wenn \omega_D < \omega_k gilt (stabiles System)?
...
Was ist die Phasenreserve \varphi_r und wie wird sie berechnet?
Was ist die Amplitudenreserve A_r und wie wird sie berechnet?
Wie liest man \varphi_r und A_r aus dem Bode-Diagramm ab?
Was bedeutet eine Phasenreserve von 0°? Von 90°?
Welche Mindestwerte werden für \varphi_r und A_r typischerweise gefordert?
Wie hängen Phasenreserve und Dämpfung des geschlossenen Kreises zusammen?
Was passiert mit \varphi_r wenn K_R erhöht wird?
...
Was ist die stationäre Regelabweichung e_\infty und wie berechnet man sie?
Wann ist e_\infty = 0 (keine bleibende Regelabweichung)?
Was ist der Systemtyp (Typ 0, Typ 1, Typ 2) und was bedeutet er?
Welche Reglerstruktur ist nötig um e_\infty = 0 bei einem Sprungeingang zu garantieren?
...
Was ist die Summenzeitkonstante T_\Sigma und wie berechnet man sie?
Wie lautet die Kessler-Formel für K_R bei D = 1/\sqrt{2}?
Was bedeutet Dämpfung D = 1/\sqrt{2} für das Überschwingen (ca. 4,3%)?
Was bedeutet D = 1 (aperiodisch optimaler Fall)?
Was ist der Unterschied zwischen D = 1 und D = 1/\sqrt{2} in der Sprungantwort?
...
Wie lautet die allgemeine Zustandsraumdarstellung \dot{x} = Ax + bu, y = c^T x?
...
Wie transformiert man das System in die Diagonalform: \dot{\eta} = \Lambda\eta + \tilde{b}u?
Was bedeutet es dass die Diagonalform entkoppelt ist?
Wie berechnet man V^{-1} für eine 2\times2-Matrix?
Wie berechnet man \tilde{b} = V^{-1}b und \tilde{c}^T = c^TV?
...
Wie hängt die Eigenfrequenz \omega_0 und Dämpfung D mit den Eigenwerten zusammen?
...
Wie wählt man die Wunscheigenwerte \lambda_1^ (aus D und \omega_0)?
...
Wie berechnet man A_R^{-1} für eine 2\times2-Matrix?
...
## Wie müssen diese Fragestellungen beantwortet werde

Alles was man in knowledge findet darf man benutzen um diese Fragen zu beantworten mit:
-mathematischen Formeln und Abbildungen, erstmal allgemeine Formeln und dann soll mithilfe von Beispielen erklärt werden wie man diese verwendet
-Abbildungen: sämtliche Diagramme, Verläufe und sonstiges ähnliches ist gemeinet
-Beispielen aus Klausuren müssen enthalten sein, sorgfälltig mithilfe der Lösungen dargestellt
-Beispiele aus den Tutorien mit Lösungen müssen enthalten sein

---

## Wie muss das Dashboard mit den Lernzetteln verknüpt sein?

Insgesamt 5 Aufgaben in der Klausur, die alle von einander abgegrenz werden sollen, es mussüen alle Teilaufgaben im Dashboard mithilfe der Inhalte im Lernzettel erklärt werden:
...
Schleifenübertragungsfunktion des offenen Regelkreises ablesen: F_o(s) = G_R(s) \cdot G_S(s)
Führungsübertragungsfunktion herleiten: F_W(s) = \frac{G_R G_S}{1 + G_R G_S}
Störübertragungsfunktion herleiten: F_Z(s) = \frac{G_{S2}}{1 + G_R G_{S1} G_{S2}}
...
Phasenreserve \varphi_r = 180° + \angle F_o(j\omega_D) ablesen
Amplitudenreserve A_r = -|F_o(j\omega_k)|_{dB} ablesen

Was als Variante vorkommt:

e) Totzeitglied
Betrag bleibt gleich, Phase sinkt um \omega \cdot T_t \cdot \frac{180°}{\pi}
...
e_\infty = \lim_{s \to 0} \frac{s \cdot W_0/s}{1 + F_o(s)} = \frac{W_0}{1 + K_o}
Bedingung für e_\infty = 0: I-Anteil in F_o(s) nötig

g) Regler aus Bode-Diagramm ablesen
PI-Regler erkennen: Anfangssteigung −20 dB/Dek., Knick bei \omega = 1/T_R
PD-Regler erkennen: Steigung +20 dB/Dek. bis zur Knickfrequenz
K_R ablesen: Betrag bei \omega = 1 (in lin. Skala umrechnen)
...
Startpunkt bei \omega \to 0: F_o(0) = K ablesen (reeller Wert auf der positiven reellen Achse)
Endpunkt bei \omega \to \infty: Winkel = -n \cdot 90° ablesen → Polüberschuss n bestimmen
...
Typische Formen: PT1 → Halbkreis, PT2 → Schlaufe, IPT1 → kommt aus -j\infty

b) F_W(s) rückwärts → Strecke oder Regler herausrechnen
Umformung: G_S = \frac{F_W}{G_R(1 - F_W)} bzw. G_R = \frac{F_W}{G_S(1-F_W)}
...
Anfangsverstärkung K ablesen (Schnitt mit \omega = 1 oder aus Niveau)
Knickfrequenzen ablesen → Zeitkonstanten T_k = 1/\omega_k
Steigungsänderungen: −20 dB/Dek. → PT1, −40 dB/Dek. → PT2
F_o(s) aufschreiben
PT2-Korrektur: bei D < 1/\sqrt{2} erscheint Überhöhung → D aus Überhöhung bestimmen
...
Formel: G_K(s) = \frac{-G_Z(s)}{G_{S1}(s)} (für Störgrößenaufschaltung)
Oder: G_K(s) zur Polkompensation bestimmen
Realisierbarkeit prüfen: Grad(Zähler) \leq Grad(Nenner)?
Ja → realisierbar ✓
...
Reglerverstärkung: K_{R,i} = \frac{n-1}{K_{S1}} \approx \frac{n}{K_{S1}}
Herleitung: F_{W,i}(s) = \frac{K_{R,i} K_{S1}}{1+K_{R,i}K_{S1}} \cdot \frac{1}{1+\frac{T_0}{1+K_{R,i}K_{S1}}s}
PT1-Näherung des geschlossenen Innenkreises: F_{W,i}(s) \approx \frac{1}{1+T_W s}

d) Außenkreis: PI-Regler nach Kessler (Betragsoptimum)
T_R = T_1 (größte Zeitkonstante der Gesamtstrecke → Polkompensation)
K_R = \frac{T_1}{2 \cdot K_S \cdot T_\Sigma} für D = 1/\sqrt{2}
T_\Sigma = Summe aller kleinen Zeitkonstanten (T_W + Parasitäre)
K_S = stationäre Streckenverstärkung (bei I-Anteil: Integrierbeiwert)
PI-Regler aufschreiben: G_{R,a}(s) = K_R \frac{1 + T_R s}{T_R s}

Was als Variante vorkommt:

e) Störgrößenaufschaltung F_{SD}
Ideale Kompensation: G_{K,SD}(s) = \frac{-G_Z(s)}{G_{S1}(s)}
Realisierbarkeit prüfen
Falls nicht realisierbar → stationäre Kompensation: G_{K,SD}(0) = \frac{-G_Z(0)}{G_{S1}(0)}
...
b) DGL herleiten
Mechanik: Freischnitt zeichnen → Newton-Gesetz \sum F = m\ddot{x}
...
Zustandsgrößen wählen (typisch: z_1 = x, z_2 = \dot{x} für 2. Ordnung)
Gleichungssystem in Matrixform bringen:

d) Eigenwerte berechnen → Stabilität und Schwingfähigkeit
\det(\lambda I - A) = 0 lösen
Für 2\times2: \lambda^2 - \text{tr}(A)\lambda + \det(A) = 0
Aussage: \text{Re}(\lambda_i) < 0 → stabil; \text{Im}(\lambda_i) \neq 0 → schwingfähig

e) Zustandsregler durch Eigenwertvorgabe
Wunscheigenwerte \lambda_1^ gegeben
Wunschpolynom aufstellen: (s-\lambda_1^) = s^2 + p_1 s + p_0
...
Eigenvektoren zu jedem \lambda_i berechnen: (A - \lambda_i I)v_i = 0
Modalmatrix aufstellen: V = [v_1 | v_2]
Inverse berechnen: V^{-1} = \frac{1}{\det V}\begin{pmatrix}d&-b\-c&a\end{pmatrix}
V^{-1} = \frac{1}{\det V}\begin{pmatrix}d&-b\-c&a\end{pmatrix}
Diagonalform: \Lambda = V^{-1}AV
Transformierte Parameter: \tilde{b} = V^{-1}b, \tilde{c}^T = c^T V
...
d) Quantisierung
Quantisierungsintervall: \Delta = \frac{x_{max} - x_{min}}{2^n - 1}
Maximaler Fehler: e_{max} = \Delta / 2
Benötigte Bits: n \geq \log_2(N_{Stufen}) → aufrunden
...
Grenzfrequenz: f_g = 1/(2\pi T)
Eignung prüfen: f_g \leq f_{abt}/2?
...
Stationärer Endwert: y_\infty = \frac{b_0}{1 - a_1}

g) Euler-Diskretisierung PT1 → Differenzengleichung
PT1 kontinuierlich: T\dot{y} + y = Ku
Euler-Vorwärts: \dot{y} \approx (y_k - y_{k-1})/\Delta t einsetzen
Koeffizienten ablesen: a_1 = \frac{T}{T+\Delta t}, b_0 = \frac{K\Delta t}{T+\Delta t}
...
-es müssen für jede Aufgabe interaktive Diagramme gegeben sein in denen man Regler, Werte und sonstiges auswählen kan und direkt Betrags phasen verlauf skizziert bekommt
-Es müssen Vorgehensweisen skizziert sein wie man bestimmte Lösungen durch Dirgramme bekommt 
